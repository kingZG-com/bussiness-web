...
**SantriCode.Labs** adalah website *Company Profile* resmi untuk layanan jasa pembuatan website, aplikasi, dan penyelesaian tugas/projek IT secara profesional, transparan, dan terpercaya. Didesain dengan estetika modern, *dark mode* bawaan, dan animasi *reveal* yang halus untuk pengalaman pengguna yang maksimal.

---

## ✨ Fitur Utama

- 🌗 **Dark & Light Mode**: Sistem tema yang dikelola menggunakan Vanilla Javascript dan terintegrasi dengan preferensi lokal (*localStorage*).
- ⚡ **Animasi Scroll Dinamis**: Menggunakan `ScrollReveal.js` untuk memunculkan elemen secara elegan saat *scrolling*.
- 💻 **Terminal Interaktif**: Simulasi animasi *typing* gaya terminal pada bagian Hero Section.
- 🧩 **Arsitektur Komponen Modular**: Meskipun menggunakan Vanilla HTML, *Navbar* dan *Footer* dipisahkan menjadi komponen modular dan dimuat secara asinkron (menggunakan `fetch() API`).
...
- **Vanilla Javascript** (Tidak ada *framework* berat)
...
Karena proyek ini menggunakan fitur `fetch()` API untuk memuat komponen *Navbar* dan *Footer*, Anda **tidak bisa** sekadar membuka `index.html` dengan *double-click* (URL `file:///`). Anda **harus** menjalankannya melalui *Local Server* untuk menghindari masalah CORS.
...
3. **Kompilasi Ulang Tailwind CSS (Jika ada perubahan desain)**
   Jika Anda memodifikasi kelas HTML atau variabel di `input.css`, *build* ulang *output*-nya:
   ```bash
